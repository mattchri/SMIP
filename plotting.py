...
def plot_tracks_mask_field_loop(track,field,Mask,name=None,plot_dir='./',figsize=(10,10),**kwargs):
    ...
    for i in range(len(time.points)):
        ...
        fig1.savefig(os.path.join(plot_dir,name+'_'+datestring_file+'.png'),dpi=200)
        plt.close()
    plt.close() 


def plot_tracks_mask_field(track,field,Mask,axes=plt.gca(),axis_extent=None,text_mass_signal=False,vmin=None,vmax=None,n_levels=50,cmap='viridis',**kwargs):
    import cartopy
    ...
    plot_field=iplt.contourf(field,coords=['longitude','latitude'],
                        levels=np.linspace(vmin,vmax,num=n_levels),axes=axes,cmap=cmap,vmin=vmin,vmax=vmax,**kwargs)
